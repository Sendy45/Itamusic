...
public class ChordTestActivity extends AppCompatActivity {
    ...
    private final String activityDailyMission = "Do 10 chords questions";
    ...
    public void nextQuestion() {
        ...
        // Display the chord
        NotesStructure chord = notesView.getRandomChord();
        notesView.setChord(chord.notes);

        answerList.add(chord.name); // Add the correct answer
        ...
        textViewAnswerList.get(0).setText(chord.name);

        // Add wrong answers
        for (int i = 0; i < 4; i++) {
            // Make sure to not override the right answer button
            if (i != ansNum) {
                String ansWrong = notesView.getRandomChord().name;

                // If answer in answer list, tries again
                while (answerList.contains(ansWrong))
                    ansWrong = notesView.getRandomChord().name;
        ...
        for (ImageButton button : ButtonAnswerList) {
            ...
            button.setOnClickListener(view -> {
                ...
                if (isCorrectAnswer) { // Success
                    ...
                    stats.incrementChordsQuestions(true);
                } else { // Failure
                    stats.incrementChordsQuestions(false);
                }
